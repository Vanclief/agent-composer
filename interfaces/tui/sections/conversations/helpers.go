...
func buildConversationColumns(totalWidth int) []table.Column {
	minSum := 0
	totalFlex := 0
	for _, spec := range conversationColumnSpecs {
		minSum += spec.min
		totalFlex += spec.flex
	}
	...
	cols := make([]table.Column, len(conversationColumnSpecs))
	for i, spec := range conversationColumnSpecs {
		width := spec.min
		if extra > 0 {
